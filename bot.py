...
def _call_groq_text(messages) -> str:
...
def _call_gemini_text(messages) -> str:
    system_text = ""
    contents = []
    for m in messages:
        if m["role"] == "system":
...
async def ask_ai(user_id: int, user_message: str) -> str:
    """
    يرسل الرسالة لأول مزوّد متاح (Groq)، وإذا خلص حده اليومي
    يتحول تلقائياً لـ Gemini بدون ما تنقطع ذاكرة المحادثة.
    """
    ...
    if groq_is_available():
        try:
            reply = _call_groq_text(messages)
    ...
    if reply is None:
        ...
        reply = _call_gemini_text(messages)
    ...
    try:
        if mime_type.startswith("image/"):
            ...
            if len(file_text) > 15000:
                file_text = file_text[:15000] + "\n... (تم اقتطاع باقي الملف لأنو طويل)"

            prompt = f"{caption}\n\nاسم الملف: {filename}\nمحتوى الملف:\n```{ext}\n{file_text}\n```"
            reply = await ask_ai(update.effective_user.id, prompt)
